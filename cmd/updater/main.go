	...
	for _, release := range config.Releases {
		...
		go func() {
			...
			if config.ModeOfOperation == common.ModeUpdate {
				ver, err := UpdateChart(ctx, &release, &config.Helm)
				if err != nil {
				...
				// todo short circuit if no changes - currently it creates branch
				// todo commit there in parallel
				err = CreateBranch(fmt.Sprintf("update/%s-%s", release.ChartName, ver))
				if err != nil {
					return
				}
...
func UpdateChart(ctx context.Context, releaseConfig *common.GithubRelease, helmSettings *common.HelmSettings) (string, error) {
	...
	return manifests.Version, nil // fixme return charts
}
